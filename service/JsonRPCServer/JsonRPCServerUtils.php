...
class JsonRPCServerUtils
{
    ...
    public function constructWhere(&$query_obj, $table = '', $module = null)
    {
        ...
        if ($table === 'users.') {
            ...
            //custom code added by Rupendra for only grouped users should show start
            ...
            if (in_array('Bostel Clients', $roles)) {
                ...
                $query = "select securitygroup_id from securitygroups_users where user_id='$current_user->id' limit 1";
                $res = $db->query($query);
                $row = $db->fetchByAssoc($res);
                
                $cond_arr[] = $table . "id IN (select user_id from securitygroups_users where securitygroup_id='".$row['securitygroup_id']."')";
                // echo "<pre>"; print_r($cond_arr); die;
            }
            //custom code added by Rupendra for only grouped users should show start
        }

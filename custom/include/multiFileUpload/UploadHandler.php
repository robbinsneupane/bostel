...
class UploadHandler
{
    ...
    {
        $this->options = array(
            'script_url' => $this->get_full_url() . '/' . $this->basename($this->get_server_var('SCRIPT_NAME')) . '?entryPoint=file_uploads',
            ...
            'custom_replacements' => [[' '], ['-']], // added custom_replacements to replace custom strings in name
    ...
    protected function trim_file_name(
        ...
        $guIdArray = explode('-', create_guid());
        $name = end($guIdArray) . '_' . trim($this->basename(stripslashes($name)), ".\x00..\x20");
        ...
        $name = str_replace($this->options['custom_replacements'][0], $this->options['custom_replacements'][1], $name);
    ...
    public function generate_response($content, $print_response = true)
    {
        ...
        if ($print_response) {
            ...
            $this->updateFileData($json); // added custom for saving data
            $this->body($json);
        }
        return $content;
    }
    ...
    public function get($print_response = true)
    {
        ...
        // custom code added by Rupendra start
        $filteredresponse = [];
        foreach ($response as $key => $keysArray) {
            if (is_array($keysArray)) {
                $filteredresponse[$key] = [];
                foreach ($keysArray as $file) {
                    if (in_array($file->name, $this->options['files'])) {
                        $filteredresponse[$key][] = $file;
                    }
                }
            } else {
                $filteredresponse[$key] = $keysArray;
            }
        }
        // custom code added by Rupendra end
        // return $this->generate_response($response, $print_response);
        return $this->generate_response($filteredresponse, $print_response);
    }
     ...
     * only for edit files not for new created
     * @param String $json
     */
    public function updateFileData($json)
    {
        if (!empty($_REQUEST['record'])) {
            ...
            if ($this->get_server_var('REQUEST_METHOD') == 'POST') {
                ...
                $newFiles['files'][] = (json_decode($json, true))['files'][0];
                $bean->attachments = json_encode($newFiles);
                $bean->save();
            } else if ($this->get_server_var('REQUEST_METHOD') == 'DELETE') {
                foreach ($files['files'] as $file) {
                    if ($_REQUEST['file'] != urlencode($file['name'])) {
                        $newFiles['files'][] = $file;
                    }
                }
                $bean->attachments = (count($newFiles['files']) > 0) ? json_encode($newFiles) : '';
                $bean->save();
            }
        }
    }

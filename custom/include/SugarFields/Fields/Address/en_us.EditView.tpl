...
{{if $displayParams.company_name }}
{{assign var="company_name" value=$displayParams.key|cat:'_company_name'}}
{{/if}}
...
<fieldset id='{{$key}}_address_fieldset'>
    ...
    <table border="0" cellspacing="1" cellpadding="0" class="edit" width="100%">
        ...
        <tr>
            <td scope='row' NOWRAP>
                {{if $smarty.request.module == 'Leads'}}
                {sugar_translate label='LBL_COPY_ADDRESS' module=''}:
                {{else}}
        ...
        {{if $displayParams.company_name }}
            <tr>
                <td id="{{$company_name}}_label" width='{{$def.templateMeta.widths[$smarty.foreach.colIteration.index].label}}%'
                    scope='row'>
                    <label for='{{$company_name}}'>{sugar_translate label='LBL_COMPANY_NAME' module='{{$module}}'}:</label>
                    {if $fields.{{$company_name}}.required || {{if $company_name|lower|in_array:$displayParams.required}}true{{else}}false{{/if}}}
                    <span class="required">{$APP.LBL_REQUIRED_SYMBOL}</span>
                    {/if}
                </td>
                <td>
                    <input type="text" name="{{$company_name}}" id="{{$company_name}}" size="{{$displayParams.size|default:30}}"
                        {{if !empty($vardef.len)}}maxlength='{{$vardef.len}}'{{/if}}
                        value='{$fields.{{$company_name}}.value}' {{if !empty($tabindex)}} tabindex="{{$tabindex}}" {{/if}}>
                </td>
            </tr>
        {{/if}}
...
<script type="text/javascript">
    SUGAR.AddressField.prototype.elems.push('company_name');

...
<div style="{if $smarty.request.action == 'Popup'}display:none{/if}">
...
<button type="button" name="btn_{{sugarvar key='name'}}" {{if !empty($tabindex)}} tabindex="{{$tabindex}}" {{/if}}  title="{$APP.LBL_SELECT_BUTTON_TITLE}"
	   ...
	   onclick='if(document.{{$form_name}}.{{$vardef.type_name}}.value != "") open_popup_modal(document.{{$form_name}}.{{$vardef.type_name}}.value, {{$displayParams.popupData}}, "single", true);'><span class="suitepicon suitepicon-action-select"></span></button>

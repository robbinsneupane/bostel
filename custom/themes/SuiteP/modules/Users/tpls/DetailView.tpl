...
<div class="detail-view">
    ...
    <ul class="nav nav-tabs">
                ...
                {{if (isset($tabDefs[$label_upper].newTab) && $tabDefs[$label_upper].newTab == true)}}
                    ...
                    {{if $tabCount == '0'}}
                        ...
                        {*{foreach name=sectionXS from=$sectionPanels key=label item=panelXS}*}
                            ...
                            <li class="m-tab-xs" role="presentation">
                                <a id="tab{{$tabCountXS}}" data-toggle="tab" onclick="customChangeFirstTab(this, 'tab-content-{{$tabCountXS}}');">
                                    {sugar_translate label='{{$label}}' module='{{$module}}'}
                                </a>
                            </li>
                        {*{/foreach}*}
                        {{counter name="tabCountXS" print=false}}
                        <li class="m-tab-xs" role="presentation">
                            <a data-toggle="tab" id="tab{{$tabCountXS}}" onclick="customChangeFirstTab(this, 'tab-content-{{$tabCountXS}}');" href="#">{sugar_translate label='LBL_ADVANCED' module='Users'}</a>
                        </li>
                        {if $SHOW_ROLES == true}
                            {{counter name="tabCountXS" print=false}}
                            <li class="m-tab-xs" role="presentation">
                                <a data-toggle="tab" id="tab{{$tabCountXS}}" onclick="customChangeFirstTab(this, 'tab-content-{{$tabCountXS}}');" href="#">{{$MOD.LBL_USER_ACCESS}}</a>
                            </li>
                        {/if}
                        </ul>
                    </li>
        ...
        {{counter name="tabCount" print=false}}
        <li role="presentation" class="hidden-xs ">
            <a data-toggle="tab" id="tab{{$tabCount}}" href="#">{sugar_translate label='LBL_ADVANCED' module='Users'}</a>
        </li>
        {if $SHOW_ROLES}
            {{counter name="tabCount" print=false}}
            <li role="presentation" class="hidden-xs ">
                <a data-toggle="tab" id="tab{{$tabCount}}" href="#">{sugar_translate label='LBL_USER_ACCESS' module='Users'}</a>
            </li>
        {/if}
        {if $config.enable_action_menu}
            <li id="tab-actions" class="dropdown">
                <a class="dropdown-toggle" data-toggle="dropdown" href="#">{sugar_translate label='LBL_LINK_ACTIONS' module='Users'}<span class="suitepicon suitepicon-action-caret"></span></a>
                {{include file="custom/themes/SuiteP/include/DetailView/actions_menu.tpl"}}
            </li>
        {/if}
    </ul>
        ...
        <div class="tab-content" style="padding: 0; border: 0;">
         {{/if}}
            ...
            {{if $useTabs}}
                ...
                {* advanced users tab *}
                <div class="tab-pane-NOBOOTSTRAPTOGGLER fade" id='tab-content-{{$tabCount}}' >
                    {{include file='custom/themes/SuiteP/modules/Users/tpls/DetailView-advanced-tab-content.tpl'}}
                </div>
                {if $SHOW_ROLES}
                    {{counter name="tabCount" print=false}}
                ...
                <script type="text/javascript">
                    ...
                    $(function(){
                        $('#content ul.nav.nav-tabs li').click(function(e){
                            if(typeof $(this).find('a').first().attr('id') != 'undefined' && !$(this).find('a').first().is(":hidden") && !$(this).hasClass('m-tab-xs')) {

...
if (!$current_user->isAdmin() && in_array('Bostel Clients', $this->roles)) {
...
// As new requirement to change suitecrm with bostelcrm start
foreach ($pageTabs as $key => $value) {
    if (stripos($value['pageTitle'], 'suitecrm') != -1) {
        $pageTabs[$key]['pageTitle'] = str_ireplace('suitecrm', 'BOSTELCRM', $value['pageTitle']);
    }
}
// As new requirement to change suitecrm with bostelcrm end

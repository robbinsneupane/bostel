...
class AOS_PDF_TemplatesViewEdit extends ViewEdit
{
    ...
    public function setFields()
    {
        ...
        $json = getJSONobj();
        $samples = array();
        if ($handle = opendir('custom/modules/AOS_PDF_Templates/samples')) {
            $sample_options_array[] = ' ';
            while (false !== ($file = readdir($handle))) {
                //custom remove account module
                if (ltrim(rtrim($file, '.php'), 'smpl_') != 'Account_Sample' && $value = ltrim(rtrim($file, '.php'), 'smpl_')) {
                    require_once('custom/modules/AOS_PDF_Templates/samples/'.$file);
                    $file = rtrim($file, '.php');
                    $file = new $file();
                    $fileArray =
                        array(
    ...
    public function displayTMCE()
    {
		...
		var df = '{$locale->getPrecedentPreference('default_date_format')}';
 		...
 		tinyMCE.init({
            ...
            language: (current_lang == 'en_us') ? 'en' : current_lang
		});

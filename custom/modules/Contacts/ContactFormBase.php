...
class ContactFormBase extends PersonFormBase
{
    ...
    public function getDuplicateQuery($focus, $prefix='')
    {
        ...
        // adding query for only same group contacts fetch by this query
        $query .= 'INNER JOIN securitygroups_records as sgr on sgr.record_id=contacts.id AND sgr.module="Contacts" 
...
EOQ;
    ...
    public function handleSave($prefix, $redirect=true, $useRequired=false)
    {
        ...
        if (
            ($redirect || (!empty($_POST['popup']) && $_POST['popup'])) && 
            !empty($_POST['is_ajax_call']) && $_POST['is_ajax_call'] == '1'
        ) {
            $json = getJSONobj();
            ...
            exit; // custom code /// exit here.. after that creating view, not need view html.
            return null;
        }

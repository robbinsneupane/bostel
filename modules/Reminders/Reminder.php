        ...
        if ($popupReminders) {
            ...
            foreach ($popupReminders as $popupReminder) {
                ...
                if (isset($relatedEvent->date_start)) {
                    ...
                    $time = $app_strings['MSG_JS_ALERT_MTG_REMINDER_NO_START_DATE'.$popupReminder->related_event_module];
                }

                //custom code for alert showing meeting type always
                $alertType = isset($app_strings['MSG_JS_ALERT_MTG_REMINDER_'.strtoupper($relatedEvent->object_name)]) ? 
                $app_strings['MSG_JS_ALERT_MTG_REMINDER_'.strtoupper($relatedEvent->object_name)] : "";

                // standard functionality
                $alert->addAlert(
                    // $app_strings['MSG_JS_ALERT_MTG_REMINDER_MEETING'], commneted orignal code
                    $alertType,
                    $meetingName,
